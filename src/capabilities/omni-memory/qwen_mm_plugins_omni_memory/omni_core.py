...
Neither copy can import the other: some harnesses install a skill by copying skill/ alone, so the
build script cannot count on the server package being near it, and the wheel does not ship skill/.
Hence a copy rather than a shared module — the same trade video-memory makes with schema.py and
embeddings.py.

tests/test_omni_memory.py::test_the_two_copies_of_omni_core_stay_identical compares every line below
the marker, so editing one copy fails until the other matches. The format two programs write and read
...
from . import config  # WIRING — the build copy reads `import env_config as config` on this line
